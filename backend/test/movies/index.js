/**
 * test/movies/index.js
 *
 * Suite interactiva de pruebas para el catálogo de películas.
 *
...
function printResponse(label, response) {
  ...
  // Si hay paginación, la mostramos separada para que sea más legible
  if (response.body.pagination) {
    console.log('Paginacion:');
    console.log(JSON.stringify(response.body.pagination, null, 2));
    console.log(`Peliculas devueltas: ${response.body.data?.length ?? 0}`);
...
async function run() {
  console.log('\n=== Movies Module - Interactive Test Suite ===\n');

  const baseInput = await ask('Base URL [http://localhost:3000/api/movies]: ');
  const baseUrl = baseInput || 'http://localhost:3000/api/movies';

  // Estado compartido entre tests para reutilizar IDs
  const state = { lastId: '' };

  while (true) {
    console.log('Selecciona una opcion:');
    console.log('1) GET /api/movies              - Catalogo basico (pagina 1)');
    console.log('2) GET /api/movies?page&limit   - Paginacion personalizada');
    console.log('3) GET /api/movies?search=      - Buscar por titulo');
    console.log('4) GET /api/movies?genre=       - Filtrar por genero');
    console.log('5) GET /api/movies?sort=        - Ordenar catalogo');
    console.log('6) GET /api/movies?search+genre+sort - Combinacion de filtros');
    console.log('7) GET /api/movies/genres/list  - Lista de generos disponibles');
    console.log('8) GET /api/movies/:id          - Detalle de una pelicula');
    console.log('9) GET /api/movies/:id          - ID invalido (debe dar 400)');
    console.log('0) Salir\n');
    ...
    try {
      if (option === '1') {
        // Catalogo basico — verifica que devuelve datos y paginacion
        ...
        const search = await ask('Titulo [mar]: ')         || 'mar';
        const genre  = await ask('Genero [Drama]: ')       || 'Drama';
        const sort   = await ask('Orden [date_desc]: ')    || 'date_desc';
        ...
        const id = (await ask(`ID de pelicula [${state.lastId || 'ninguno guardado'}]: `)) || state.lastId;
        if (!id) {
          console.log('\nEjecuta primero el test 1 para guardar un ID automaticamente.\n');
          continue;
        }
        const res = await requestJson(`${baseUrl}/${id}`);
        printResponse(`DETALLE pelicula id="${id}"`, res);

      } else if (option === '9') {
        // Debe devolver 400 con mensaje "Invalid movie id"
